...
const int Game::randomCreation = 2;
const int Game::randomPowerUp = 3;
...
void Game::createObjects() {
    if (objectClock.getElapsedTime().asSeconds() >= creationRate) {
        if (counter % 5 == 0 && random(randomCreation) == 1) {
            std::unique_ptr<Enemy> enemy = factoryE.createEnemy(EnemyType::ShootingEnemy);
            if (random(randomCreation) != 0)
                enemy->setEnemySpeedY(speed.y);
            else
                enemy->setEnemySpeedY(-speed.y);
            enemy->setPosition(sf::Vector2f(2 * windowSize.x, random(maxY)));
        ...
        if (counter % 2 == 0 && random(randomCreation) == 1 && !isEnemyCreated) {
...
/* funzione randomica
limit dipende da dove viene chiamata */
